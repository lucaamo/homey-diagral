...
class DiagralClient {
  ...
  async getWebhook() {
    await this.ensureReadyForSignedCalls();
    return this.request(`/webhooks/${this.serialId}/subscription`, {
      method: 'GET',
      headers: this.signedHeaders(false),
    });
  }

  async upsertWebhook(webhookUrl) {
    const existing = await this.getWebhook().catch(err => {
      const message = String(err?.message || err || '');
      if (
        message.includes('404') ||
        message.includes('No subscription found') ||
        message.includes('There is no subscription for')
      ) {
        return null;
      }
      throw err;
  ...
  async partialStartSystem(id = 1) {
    const partialId = Number(id);
    if (![1, 2].includes(partialId)) {
      throw new Error('Partial start id must be 1 or 2');
    }

    await this.ensureReadyForSignedCalls();
    return this.request(`/systems/${this.serialId}/partial_start_${partialId}`, {
      method: 'POST',
      headers: this.signedHeaders(true),
    });
  }
  ...
  async getDevicesConfiguration() {
    const config = await this.getConfiguration();
    const deviceTypes = ['cameras', 'commands', 'sensors', 'sirens', 'transceivers', 'transmitters'];

    return deviceTypes.reduce((result, type) => {
      const devices = config?.[type];
      result[type] = Array.isArray(devices) ? devices : [];
      return result;
    }, {});
  }
